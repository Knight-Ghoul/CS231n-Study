...
class TwoLayerNet(object):
    ...
    一个双层全连接神经网络。 网络具有N的输入维度，H的隐藏层维度，并且对C类执行分类。
     我们使用softmax损失函数和权重矩阵上的L2正则化训练网络。 网络在第一个完全连接的层之后使用ReLU非线性。
...
     第二个完全连接层的输出是每个类的分数。
    ...
    def __init__(self, input_size, hidden_size, output_size, std=1e-4):
        ...
        初始化模型。 权重被初始化为小的随机值，偏差被初始化为零。
        权重和偏差存储在变量self.params中，这是一个包含以下键的字典：
